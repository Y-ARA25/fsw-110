...
let partsList = '<ol>'
parts.forEach(item) {
    partsList += `<li>${item.qty} (${item.partNbr}) - ${item.partDescr}</li>`
}
partsList += '</ol>'

document.getElementById('detailsList').innerHTML = partsList

detailsList.appendChild(partsList)
